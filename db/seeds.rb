...
users.each do |name|
  ...
  user.user_numbers << apps.map do |app|
    UserNumber.create(plivo_service.create_sip_endpoint("#{user.name}#{app.capitalize}", FFaker::Internet.password))
  end
end

lines.each_with_index do |line, index|
  aviato.company_numbers <<  CompanyNumber.create(plivo_service.create_sip_endpoint(line, FFaker::Internet.password))
end
